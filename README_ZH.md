...
更新源是 [本仓库的 GitHub Releases](https://github.com/FengBujue0104/cc-switch-cli-fbj/releases)，不是上游 SaladDay。`cc-switch update` 会用 `checksums.txt`（或 GitHub 资源 digest）做 SHA-256 校验。仍显示 `5.10.5` 的二进制会把 `v5.10.5-fbj.1` 当成更旧的预发布号而跳过；先用上面的脚本重装一次，之后的 `v5.10.6` 等标签即可自更新。
