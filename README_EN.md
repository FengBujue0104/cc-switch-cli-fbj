...
- **Smaller command surface.** Visible top-level commands are `auth`, `provider`, `use`, `config`, `proxy`, `settings`, `start`, `daemon`, `env`, `update`, `interactive`, `completions` (`start` / `daemon` are Unix only; `proxy` is a default-on cargo feature, and `--no-default-features --features cli` drops the proxy commands); the `skills`, `mcp`, `sessions`, and `usage` entry points are removed.
