...
pub struct ConfigSnapshot {
    ...
    pub global_outbound_proxy: Option<crate::services::GlobalOutboundProxyConfig>,
...
impl ProxySnapshot {
    ...
    pub fn takeover_enabled_for(&self, app_type: &AppType) -> Option<bool> {
        // Gemini 已从本构建移除：不再为它查一次数据库，`takeover_enabled_for` 对
        // 已删 harness 一律回答 None（"这个应用没有代理控制"），和
        // `cli/commands/proxy.rs` 的接管闸门保持一致。
        match app_type {
...
impl ConfigSnapshot {
    fn loading_projection(&self, app_type: &AppType) -> Self {
        Self {
            ...
            global_outbound_proxy: self.global_outbound_proxy.clone(),
...
fn load_config_snapshot(state: &AppState, app_type: &AppType) -> Result<ConfigSnapshot, AppError> {
    ...
    Ok(ConfigSnapshot {
        ...
        global_outbound_proxy: crate::services::global_proxy::load(&state.db)?,
...
"#
    ...
    fn proxy_snapshot_returns_app_specific_takeover_state() {
        ...
        // 已删的 harness 一律是 None：它们不再有代理路由，也就不存在"接管中"。
        // Gemini 以前会回答 `Some(gemini_takeover)`，现在快照里连这个字段都没有。
        assert_eq!(snapshot.takeover_enabled_for(&AppType::Gemini), None);
